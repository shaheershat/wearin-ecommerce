...
def release_expired_reservations_task():
    ...
    for product in expired_products:
        ...
        user_who_reserved = product.reserved_by_user 
...
def send_return_processed_email(return_request_id, status):
    ...
    print(f"DEBUGGING: Task received return_request_id={return_request_id}, status='{status}'") 
    try:
        from core.models import ReturnRequest, Order, OrderItem, Wallet, WalletTransaction 
        ...
        if not order:
            ...
            print(f"DEBUGGING: SKIPPING EMAIL: Order for ReturnRequest ID {return_request_id} not found.") 
            return
